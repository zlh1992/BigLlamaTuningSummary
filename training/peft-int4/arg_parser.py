...
def parse_commandline():
    ...
    # Config args group
    parser_config.add_argument("--ds_type", choices=["txt", "alpaca", "gpt4all", "bluemoon"], default="alpaca",
                               required=False,
    ...
    parser_training.add_argument("--grad_chckpt", action="store_true", required=False,
                                 help="Use gradient checkpoint. For 30B model. Default: %(default)s")
    parser_training.add_argument("--grad_chckpt_ratio", default=1, type=float,
                                 help="Gradient checkpoint ratio. Default: %(default)s")
    parser_training.add_argument("--val_set_size", default=0.2, type=float,
    ...
    parser_training.add_argument("-c", "--checkpoint", action="store_true",
                                 help="Produce checkpoint instead of LoRA. Default: %(default)s")
    parser_training.add_argument("--skip", action="store_true",
                                 help="Don't train model. Can be useful to produce checkpoint from existing LoRA. Default: %(default)s")
    parser_training.add_argument("--verbose", action="store_true",
    ...
    parser_training.add_argument("--use_eos_token", default=1, type=int,
    ...
    # Multi GPU Support
    parser_training.add_argument("--local_rank", type=int, default=0,
                                 help="local rank if using torch.distributed.launch")
    parser_training.add_argument("--max_memory", type=int, default=16,
                                 help="max memory for training model in each GPU device optimal value is half of the memory of GPU devices")
    parser_training.add_argument("--max_memory_cpu", type=int, default=48,
                                 help="max memory for training model in cpu optimal value is three times of --max_memory")

    # Flash Attention
    parser_training.add_argument("--flash_attention", action="store_true",
                                 help="enables flash attention, can improve performance and reduce VRAM use")
    parser_training.add_argument("--xformers", action="store_true",
