...
class DataArguments:
    ...
    dataset: str = field(
        default='/data/Workspace/zlh/llm_cls_test/data_test/test_data.json',
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        for k, v in tokenized_sources_with_prompt.items():
            if self.padding_side == 'right':
                ...
                tokenized_targets[k] = tokenized_targets[k][:self.target_max_len]
            else:
                tokenized_sources_with_prompt[k] = tokenized_sources_with_prompt[k][-self.target_max_len:]
                tokenized_targets[k] = tokenized_targets[k][-self.target_max_len:]
        ...
        if self.padding_side == 'right':
            ...
            labels = [torch.flip(i, (0,)) for i in labels]
            labels = pad_sequence(labels, batch_first=True,
                                  padding_value=IGNORE_INDEX) if not self.predict_with_generate else None
...
def train():
    ...
    print(f'check task( do_train:{args.do_train} do_eval:{args.do_eval} do_predict:{args.do_predict})')
