...
def main():
    ...
    if args.lora_dim > 0:
        ...
        print("lora_dim: {}, lora_alpha: {}, lora_scaling: {}, lora_dropout: {}".format(args.lora_dim, args.lora_alpha,
                                                                                        args.lora_alpha / args.lora_dim,
                                                                                        args.lora_dropout))

        model = convert_linear_layer_to_lora(model, lora_module_name=lora_module_name, lora_dim=args.lora_dim,
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if step % args.log_step == 0:
                ...
                print_rank_0(
                    f"epoch {epoch} step {step} train loss {sum(training_step_losses) / len(training_step_losses)}, log_step {args.log_step}, log_step_time {log_step_time}, train schedule {step / len(train_dataloader)}, estimated to consume {(len(train_dataloader) - step) * (log_step_time / args.log_step)}",
                    args.global_rank)
                training_step_losses = []
