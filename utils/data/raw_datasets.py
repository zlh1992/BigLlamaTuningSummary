...
class unicorn(PromptRawDataset):
    def __init__(self, output_path, seed, local_rank, data_input_path, eval_split=None):
        super().__init__(output_path, seed, local_rank, data_input_path)
        self.dataset_name = "unicorn"
        self.dataset_name_clean = "unicorn"
        self.data_path = data_input_path
        print("use dataset:", data_input_path)

        with open(self.data_path, "r") as f:
            data = json.load(f)
        train_data = Dataset.from_list(data)
        with open(self.data_path.replace('train', 'valid'), "r") as f:
            data = json.load(f)
        test_data = Dataset.from_list(data)
        self.raw_datasets = {'train': train_data,
                             'test': test_data}  # data.train_test_split(test_size=eval_split, shuffle=True)

    def get_len_train_data(self):
        return len(self.raw_datasets["train"])

    def get_len_eval_data(self):
        return len(self.raw_datasets["test"])
...
"""
    ...
    def get_prompt_and_chosen_vicuna(self, sample, sentence_from):
        return "A chat between a curious user and an artificial intelligence assistant. \nThe assistant gives helpful, detailed, and polite answers to the user's questions." + " " \
               + "USR" + ": " + sample["value"] + " " + "ASSISTANT" + ": " if sentence_from == 'human' else sample[
            "value"]
