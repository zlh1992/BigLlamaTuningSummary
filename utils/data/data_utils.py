...
def get_raw_dataset(dataset_name, output_path, seed, local_rank):
    if "Dahoas/rm-static" in dataset_name:
        ...
        return raw_datasets.LmqgQagjaquadDataset(output_path, seed, local_rank,
                                                 dataset_name)
    elif "cls" or 'key_code' in dataset_name:
        return raw_datasets.unicorn(output_path, seed, local_rank,
                                    dataset_name)
    else:
        raise RuntimeError(
...
class PromptDataset(Dataset):
    ...
    def __init__(self, prompt_dataset, chosen_dataset, reject_dataset,
                 pad_token_id=0, train_phase=2) -> None:
        super().__init__()
    ...
    def __getitem__(self, idx):
        if self.train_phase == 1:
            return {
                ...
                "labels": self.chosen_dataset[idx]["input_ids"]  # sft
            }
        elif self.train_phase == 2:
            return {
                "input_ids": self.chosen_dataset[idx]["input_ids"], \
                "attention_mask": self.chosen_dataset[idx]["attention_mask"], \
                "labels": self.chosen_dataset[idx]["labels"]  # cls
            }
...
def pad_tensors_to_max_length(input_tensor, max_length, pad_token_id):
    ...
    padded_tensor[-input_tensor.shape[0]:] = input_tensor  # left
    return padded_tensor


def create_dataset_split(current_dataset, raw_dataset, train_phase, tokenizer,
                         end_of_conversation_token, max_seq_len):
    def _cls_tokenize(source, f_idx):
        '''
        add speaker and concatenate the sentences
        {
            "id": "uniq_sample_id",
            "conversations": [
                {"input": "human:你好\nassistant:你好，有什么可以帮助你的吗？", "score": "1"}
            ]
        }
        '''
        conversation = ''
    ...
    if train_phase == 1:
        for i, tmp_data in enumerate(current_dataset):
            ...
            if chosen_sentence is not None:
                # chosen_sentence += end_of_conversation_token  # llama 不需要
        ...
        for i, tmp_data in enumerate(current_dataset):
            # tokenize the text
            input_ids, labels, conversation = _cls_tokenize(tmp_data, i)
            ...
            chosen_token = {
                ...
                "attention_mask": pad_tensors_to_max_length(attention_mask, max_seq_len, 0),
                "labels": labels
            }
            if i == 0:
